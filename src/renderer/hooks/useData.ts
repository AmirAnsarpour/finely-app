...
export function useData() {
  ...
  const loading    = tx.loading    || cat.loading    || sett.loading    || inst.loading    || goal.loading
  const refreshing = tx.refreshing || cat.refreshing || sett.refreshing || inst.refreshing || goal.refreshing
  const error      = tx.error      || cat.error      || sett.error      || inst.error      || goal.error

  const refreshData = useCallback(async () => {
    await Promise.all([
      tx.loadData(), cat.loadData(), sett.loadData(), inst.loadData(), goal.loadData(),
    ])
  }, [tx.loadData, cat.loadData, sett.loadData, inst.loadData, goal.loadData])
